...
Microservices are an extension of service-oriented architecture. The main objective is that each of these microservices provides a well-defined and clear scope of functionalities, which can be accessed by another microservice using \textit{remote procedure calls} \cite{nelson1981remote}. By using this pattern it can be established clear ownership of each microservice by different teams where they are responsible for the development, maintenance, and deployment. This is a clear advantage over the classic monolithic architecture, since in these, although the development tasks may be divided among different teams, the deployment of a single functionality involves the deployment of the entire system, which is then prone to cause major problems that can lead to the complete shutdown of the application.\\
...
\subsection{Current sofware architecture description}
...
The architecture of UnityAds was initially developed by a Finnish startup, Applifier, acquired by Unity Technologies in 2014 \cite{UnityBuyApplifier}, and it can be seen in Figure \ref{fig:background:applifier-architecture}.
...
\label{sec:microservices-monitoring}
...
For the elicitation of requirements, the selected method is stakeholders identification \cite{mitchell1997toward}, which consists of selecting individuals or groups interested in the product or function to be developed and asking them what the desired characteristics they expect from the product are. This method is the most appropriate for this thesis since it will only have an internal impact as external customers will not use it. The list of stakeholders for this project is given in Section \ref{sec:problem-definition}.\\

\subsection{Requirements recording and gathering}
...
\begin{itemize}
    ...
    \item This format defines a contract on what the endpoint handlers should do when managing the responsibilities. The engineers that are going to implement the proxy should fulfill the specifications of the standards, and the endpoints consumer should rely on those responsibilities.
    \item There are some literature examples of composable gateway logic \cite{UberGateway}; this format can be the starting point of this automated gateway generator. 
\end{itemize}
...
\begin{itemize}
    ...
    \item[\textbf{C2}] \textbf{Data access}: This criterion sets how easy it is to query the data stored using an standard query language such as SQL.
...
Based on the three criteria, table \ref{tab:data-souces} contains a summary of the comparison between the three technologies. Based on this table and for the particular case of the Unity Services Gateway, the selected technology to continue with is BigQuery. The main factor for making that decision was that it is already integrated into the cloud environment, and it is already being used in other parts of the technical stack.\\
...
\subsubsection{Datastorage implementation with BigQuery}
...
As Unity is using Terraform for configuring all the infrastructure, for creating this sink, a new Terraform resource (with type \texttt{google\_logging\_project\_sink} \cite{TerraformGoogleSink}) has to be added. Then the configuration has to be applied to take effect. On Terraform, the structure for creating the log sink is as follows:\\
...
\begin{itemize}
    \item \textbf{Time interval}: this variable is not explicitly created for this dashboard, but it is a default variable on Grafana. It defines the time range that will be selected for showing the time series data.
...
\subsubsection*{Small number of microservices (1 - 20)}
...
\subsubsection*{Medium number of microservices (20 - 50)}
...
In that case, the most crucial step is defining the format for determining the routes that the gateway will expose to the public and how to implement the high-level responsibilities. Considering this, one sensible approach will be to select one of the existing industry-standard solutions for the gateway (Section \ref{sec:gateways-comparison}), as it will dramatically reduce the implementation and maintenance time. One possible solution to monitoring it will be to adapt the knowledge exposed in the technical solution of this thesis (Section \ref{sec:technical-solution}) to the particular case of the gateway technology that they are running.


\subsubsection*{Big number of microservices (More than 50)}
...
The first one is about the different types of errors that were mapped. As it was stated in section \ref{sec:error-event-definition}, for this project, only rate-limiting responsibility is considered for the monitoring; this is because it was the error that project managers and teams were more worried about. Nevertheless, other types of errors can be handled, and that can be done by extending the error type and using the custom function introduced in section \ref{sec:error-event-definition}.\\

The second improvement idea will be to create a gateway composable logic tool. Having the standard defined in section \ref{sec:gateway-configuration-file}, the next step can be to create a solution that injects the handling of those high-level responsibilities on the gateway at execution time to minimize the effort for defining new routes and resources.\\
